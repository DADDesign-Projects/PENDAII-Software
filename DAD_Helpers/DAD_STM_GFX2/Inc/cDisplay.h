...
DadGFX::sFIFO_Data      NO_CACHE_RAM DisplayName##FIFO;									 /* FIFO pour émission SPI en DMA*/\
DadGFX::sColor          NO_CACHE_RAM DisplayName##BlocFrame[BLOC_HEIGHT][BLOC_WIDTH];    /* Frame Blocs*/\
DadGFX::cDisplay 					 DisplayName;					                     /* Screen */

#define INIT_DISPLAY(DisplayName, hSPI) DisplayName.init(hSPI, &DisplayName##FIFO, &DisplayName##BlocFrame[0][0])

#define DECLARE_LAYER(LayerName, ValWidth, ValHeight) \
static DadGFX::sColor SDRAM_SECTION __Layer##LayerName[ValWidth][ValHeight]; \
